...
export default class StepOneScreen extends Component {
    ...
    getOfflineData = async () => {
        try {
            ...
            this.setState({ cropNameLanguageChangeArray: cropSpecificSteps[7].cropData, cropSpecificSteps })
            this.setState({ numberOfSteps: cropSpecificSteps.length })
            var stepId = cropSpecificSteps[7]._id
            ...
            this.setState({ englishDescription: cropSpecificSteps[7].english })
            this.setState({ hindiDescription: cropSpecificSteps[7].hindi })
            this.setState({ odiaDescription: cropSpecificSteps[7].odia })
            this.setState({ hoDescription: cropSpecificSteps[7].ho })
            this.setState({ santhaliDescription: cropSpecificSteps[7].santhali })
            this.setState({ englishTitleDescription: cropSpecificSteps[7].nameEnglish })
            this.setState({ hindiTitleDescription: cropSpecificSteps[7].nameHindi })
            this.setState({ odiaTitleDescription: cropSpecificSteps[7].nameOdia })
            this.setState({ hoTitleDescription: cropSpecificSteps[7].nameHo })
            this.setState({ santhaliTitleDescription: cropSpecificSteps[7].nameSanthali })
            this.setState({ stepImage: cropSpecificSteps[7].imageFile })
        } catch (error) {
            console.log(error)
        }
    }
    ...
    setValueToPatch = async () => {
        try {
            ...
            specificPatch.step8 = this.state.sumOfMaterials
            await AsyncStorage.setItem('user', JSON.stringify(parsed))
            console.log(specificPatch, "step8")
        } catch (error) {
    ...
    setStepDataIntoPatch = async () => {
        ...
        if (
            (
                ...
                this.state.cropSpecificSteps[7].videoFile &&
                files.includes("video_" + this.state.cropSpecificSteps[7].videoFile)
            ...
            try {
                ...
                if (this.state.saveButtonClicked === false && this.state.multipleMaterials.length > 0) {
                    ...
                    if (cropSpecificSteps[8] === undefined) {
                ...
                AsyncStorage.setItem("jump", "StepEightScreen")
            } catch (error) {
                console.log(error)
            }
        }
                ...
                <ScrollView nestedScrollEnabled={true}>
...

                    {
                        ...
                        [
                            ...
                            <VideoComponent
                                asyncVideoFileName={this.state.cropSpecificSteps[7].videoFile}
                    ...
                    <View style={{ backgroundColor: BaseColor.Red, width: widthToDp("90%"), height: heightToDp("60%"), alignSelf: 'center', marginTop: heightToDp("0%"), borderRadius: 10 }}>
                        <LabelComponent
                            stepName={this.state.descriptionLabel}
                            asyncKey={"cropSteps"}
                            index={7}
                        />
                        <ScrollView nestedScrollEnabled style={{ backgroundColor: 'white', width: widthToDp("90%"), height: widthToDp("50%"), borderBottomLeftRadius: 10, borderBottomRightRadius: 10, marginTop: heightToDp("0%") }}>
                            {
